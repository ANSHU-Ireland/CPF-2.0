...
## Capacity model — load test results (Delivery Plan Step 48, 2026-07-26)

**Caveat, read first**: every number below was measured on a single-node
local throwaway Postgres and a single API process on a developer laptop,
with a synthetic 1,000-row dataset, no network hop, no connection pooling
limits, and no concurrent competing tenants. **Local numbers are not
production numbers** — they validate that the query and endpoint shapes are
sound, not that production capacity is proven. Real capacity planning
requires a staging-environment run against production-shaped infrastructure
before any pilot commitment.

Tooling: [k6 v2.1.0](https://k6.io); fixtures generated by
[`tools/load/seed-load-test.mjs`](../../tools/load/seed-load-test.mjs) (1,000
`assessment_sessions` rows across a realistic status distribution, plus one
fully finalised review for the evidence-profile read). Each scenario ran
`constant-arrival-rate` at 10 req/s for 30s (2 min for the candidate event
burst, per plan), threshold `p(95)<500ms` and `http_req_failed rate<0.01`.

| Script | Endpoint | p95 | Failures | Threshold |
|---|---|---|---|---|
| [`login.js`](../../tools/load/login.js) | `POST /v1/auth/login` | 366.67ms | 0% | ✅ pass (bcrypt hashing dominates; expected) |
| [`sessions-list.js`](../../tools/load/sessions-list.js) | `GET /v1/orgs/:orgId/sessions` (1,000 seeded sessions) | 13.92ms | 0% | ✅ pass |
| [`profile-read.js`](../../tools/load/profile-read.js) | `GET /v1/orgs/:orgId/sessions/:sessionId/evidence-profile` | 7.25ms | 0% | ✅ pass |
| [`candidate-event-burst.js`](../../tools/load/candidate-event-burst.js) | `POST /v1/candidate/:token/events` (2 min sustained) | 3.41ms | 0% | ✅ pass |

All four scenarios passed their thresholds on the first real run — no
fabricated or retried-until-passing numbers.

**`EXPLAIN ANALYZE` finding**: `GET /v1/orgs/:orgId/sessions` had no index
beyond the primary key on `assessment_sessions`; its row-level-security
predicate (`organisation_id = current_org_id()`) forced a sequential scan.
At 1,002 rows this was already fast (Execution Time ≈1.0ms — the planner
correctly prefers a seq scan over an index at this tiny scale), but a seq
scan degrades linearly with total session volume across *all* organisations,
not just the reading org's own rows. Added
[migration `0021_sessions_list_index.sql`](../../packages/db/migrations/0021_sessions_list_index.sql):
`CREATE INDEX idx_assessment_sessions_org_created ON assessment_sessions
(organisation_id, created_at DESC)` — a proactive production-scale
safeguard; the local 1,002-row re-run still (correctly) shows a seq scan,
since the planner already knows that's cheaper at this size. The audit
search query (`GET /v1/orgs/:orgId/audit/search`) already had a matching
composite index (`idx_audit_log_org_time`) from migration 0001, so no change
was needed there.

**Not measured in this cycle (disclosed gap)**: concurrent multi-tenant
load, connection-pool exhaustion under burst, and any network-latency
component of a real deployment — all require a staging-environment run and
are out of scope for a local developer-machine load test.
